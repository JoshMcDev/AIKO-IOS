...
final class UserPreferenceStore {
    ...
    private func isCacheValid() -> Bool {
        return Date().timeIntervalSince(lastCacheUpdate) < cacheExpiration
    }
    ...
    private func generateCacheKey(for preference: UserPreference) -> String {
        return "\(preference.userId)_\(preference.category)_\(preference.key)"
    }
    ...
    private func persistPreference(_ preference: UserPreference) async throws {
        ...
        do {
            ...
            if let existingEntity = entities.first {
                ...
                entity = PreferenceEntity(context: context)
            }
...
struct PreferenceContext {
    ...
    init(formType: String? = nil, documentType: String? = nil, workflowPhase: String? = nil) {
        ...
        if let workflowPhase = workflowPhase {
...
extension UserPreference {
    init?(from entity: PreferenceEntity) {
        guard let id = entity.id,
              ...
              let valueType = entity.valueType else {
            return nil
        }
        ...
        // Decode value
        do {
            self.value = try decodeValue(from: valueData, type: valueType)
        } catch {
            return nil
        }
