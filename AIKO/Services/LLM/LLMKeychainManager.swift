...
final class LLMKeychainManager: ObservableObject {
    ...
    private func keychainAccount(for provider: LLMProvider) -> String {
        return "llm.apikey.\(provider.rawValue)"
    }
}
...
enum KeychainError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .storeFailed(let status):
            return "Failed to store API key: \(keychainErrorMessage(status))"
        case .retrieveFailed(let status):
            return "Failed to retrieve API key: \(keychainErrorMessage(status))"
        case .updateFailed(let status):
            return "Failed to update API key: \(keychainErrorMessage(status))"
        case .deleteFailed(let status):
            return "Failed to delete API key: \(keychainErrorMessage(status))"
        case .dataCorrupted:
            return "API key data is corrupted"
        case .biometricAuthenticationFailed:
