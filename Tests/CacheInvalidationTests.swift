...
final class CacheInvalidationTests: XCTestCase {
    ...
    func testDependencyBasedInvalidation() async throws {
        ...
        await XCTAssertNotNil(cache.get(vendorAction))
        await XCTAssertNotNil(cache.get(contractAction1))
        await XCTAssertNotNil(cache.get(contractAction2))
    ...
    func testPatternBasedInvalidation() async throws {
        ...
        // Verify all are cached
        for action in testActions {
            await XCTAssertNotNil(cache.get(action))
        }
        ...
        await XCTAssertNil(cache.get(testActions[0]))
        await XCTAssertNil(cache.get(testActions[1]))
        await XCTAssertNotNil(cache.get(testActions[2]))
    }
