...
final class OCRAccuracyTest: XCTestCase {
    ...
    private func compareExtractedData(image: ExtractedData, pdf: ExtractedData) {
        ...
        // Compare prices
        let imagePrices = image.entities.filter { $0.type == .price }.map { $0.value }
        let pdfPrices = pdf.entities.filter { $0.type == .price }.map { $0.value }
...
extension OCRAccuracyTest {
    ...
    private func measureTime<T>(operation: () async throws -> T) async throws -> TimeInterval {
        let start = Date()
