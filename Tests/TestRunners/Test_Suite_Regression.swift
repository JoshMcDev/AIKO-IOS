...
extension RegressionTestSuite {
    static func runAllTests() async throws -> TestResults {
        ...
        results.record(test: "All Document Types", 
                      result: await suite.runTest(testAllDocumentTypesGenerate))
        results.record(test: "PWS SLA Section", 
                      result: await suite.runTest(testPWSIncludesSLASection))
        results.record(test: "OT Agreement", 
                      result: await suite.runTest(testOTAgreementGeneration))
        
        // Compliance Tests
        results.record(test: "FAR Compliance", 
                      result: await suite.runTest(testFARComplianceAllDocumentTypes))
        results.record(test: "CMMC Tracking", 
                      result: await suite.runTest(testCMMCComplianceTracking))
        results.record(test: "FAR Part 12", 
                      result: await suite.runTest(testFARPart12CommercialItems))
        
        // Workflow Tests
        results.record(test: "Workflow States", 
                      result: await suite.runTest(testWorkflowStateTransitions))
        
        // Service Tests
        results.record(test: "Dependencies", 
                      result: await suite.runTest(testDocumentDependencyService))
        results.record(test: "Pattern Tracking", 
                      result: await suite.runTest(testUserPatternTracking))
        
        // Data Tests
        results.record(test: "Acquisition CRUD", 
                      result: await suite.runTest(testAcquisitionCRUD))
        
        // Export Tests
        results.record(test: "Document Export", 
                      result: await suite.runTest(testDocumentExport))
