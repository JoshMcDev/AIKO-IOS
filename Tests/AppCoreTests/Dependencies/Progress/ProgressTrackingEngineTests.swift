...
final class ProgressTrackingEngineTests: XCTestCase {
    ...
    func testStartSessionCreatesStream() async {
        ...
        let stream = await engine.startSession(sessionId: sessionId)
    ...
    func testStartSessionWithCustomConfig() async {
        ...
        let stream = await engine.startSession(sessionId: sessionId, config: config)
        var iterator = stream.makeAsyncIterator()
    ...
    func testGetSessionStateAfterStart() async {
        ...
        _ = await engine.startSession(sessionId: sessionId)
        
        let state = await engine.getSessionState(sessionId)
    ...
    func testGetActiveSessionsIncludesStartedSession() async {
        ...
        let activeSessions = await engine.getActiveSessions()
    ...
    func testSubmitProgressUpdate() async {
        ...
        await engine.submitUpdate(progressUpdate)
    ...
    func testSubmitErrorUpdate() async {
        ...
        await engine.submitUpdate(errorUpdate)
    ...
    func testRejectInvalidProgressValues() async {
        ...
        await engine.submitUpdate(invalidUpdate)
        ...
        await engine.submitUpdate(validUpdate)
    ...
    func testRejectBackwardsProgress() async {
        ...
        await engine.submitUpdate(forwardUpdate)
        ...
        await engine.submitUpdate(backwardsUpdate)
        ...
        await engine.submitUpdate(anotherUpdate)
    ...
    func testMultipleConcurrentSessions() async {
        ...
        let stream1 = await engine.startSession(sessionId: sessionId1)
        let stream2 = await engine.startSession(sessionId: sessionId2)
    ...
    func testSessionIsolation() async {
        ...
        await engine.submitUpdate(update1)
        ...
        await engine.submitUpdate(update2)
    ...
    func testCleanupExpiredSessions() async {
        ...
        _ = await engine.startSession(sessionId: sessionId, config: config)
        
        // Session should be active initially
        var activeSessions = await engine.getActiveSessions()
        XCTAssertTrue(activeSessions.contains(sessionId))
        ...
        activeSessions = await engine.getActiveSessions()
        XCTAssertFalse(activeSessions.contains(sessionId))
    }
}