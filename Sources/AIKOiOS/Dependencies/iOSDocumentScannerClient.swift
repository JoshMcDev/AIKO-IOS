        ...
        static let live: DocumentScannerClient = {
            ...
            return DocumentScannerClient(
                ...
                isScanningAvailable: {
                    ...
                    true // VisionKit availability is determined at runtime
                },
    ...
    extension iOSDocumentScannerClient {
        ...
        private static func checkCameraPermissions() async -> Bool {
                ...
                // On simulator, assume camera permissions are granted for testing
                return true
            #else
                // Check actual camera permissions on device
        ...
        private static func performEnhancedOCR(on imageData: Data) async throws -> OCRResult {
            ...
            guard let enhancedImage = UIImage(data: enhancedImageData),
                  let enhancedCGImage = enhancedImage.cgImage
            else {
        ...
        private static func detectFormFields(
            textObservations: [VNRecognizedTextObservation],
            rectangleObservations _: [VNRectangleObservation]
        ) -> [FormField] {
            var formFields: [FormField] = []
