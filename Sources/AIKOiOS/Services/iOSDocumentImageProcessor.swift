...
private final class LiveDocumentImageProcessor: Sendable {
    ...
    init() {
        ...
        self.queue = DispatchQueue(label: "document.image.processor", qos: .userInitiated)
    }
    ...
    private func performProcessing(
        ...
        let totalSteps = mode == .enhanced ? 6 : 3
    ...
    private func performPreprocessing(_ image: CIImage, options: ProcessingOptions) throws -> CIImage {
        var result = image
    ...
    private func performDenoising(_ image: CIImage, options: ProcessingOptions) throws -> CIImage {
        var result = image
    ...
    private func performBasicSharpening(_ image: CIImage, options: ProcessingOptions) throws -> CIImage {
    ...
    private func performAdvancedSharpening(_ image: CIImage, options: ProcessingOptions) throws -> CIImage {
        var result = image
    ...
    private func performOCROptimization(_ image: CIImage, options: ProcessingOptions) throws -> CIImage {
        var result = image
    ...
    private func analyzeQuality(_ processedImage: CIImage, originalImage: CIImage) throws -> QualityMetrics {
    ...
    private func calculateImageStatistics(_ image: CIImage) -> (mean: Double, standardDeviation: Double) {
        ...
        guard let grayscaleImage = grayscaleFilter.outputImage,
              let cgImage = context.createCGImage(grayscaleImage, from: grayscaleImage.extent) else {
    ...
    func estimateProcessingTime(_ imageData: Data, mode: ProcessingMode) async throws -> TimeInterval {
        ...
        let performanceFactor: Double = 1.0 // Could be adjusted based on device capabilities
        
        return baseTime * sizeMultiplier * performanceFactor
    }
    
    func isProcessingModeAvailable(_ mode: ProcessingMode) -> Bool {
