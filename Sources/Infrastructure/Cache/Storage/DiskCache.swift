...
public actor DiskCache: OfflineCacheProtocol {
    ...
    public func store(_ object: some Codable, forKey key: String, ttl: TimeInterval? = nil) async throws {
    ...
    /// Store data in cache
    public func set(_ key: String, value: some Codable, ttl: TimeInterval? = nil) async throws {
    ...
    public func get<T: Codable>(_ key: String, type _: T.Type) async throws -> T? {
    ...
    public func getStatistics() -> DiskCacheStatistics {
        ...
        let expiredCount = cacheIndex.values.filter(\.isExpired).count
    ...
    func checkHealth() async -> CacheHealthStatus {
        ...
        _ = cacheIndex.values.filter(\.isExpired).count
        let totalSize = cacheIndex.values.reduce(0) { $0 + $1.size }
    ...
    func exportAllData() async -> SendableExportData {
        ...
        for (key, entry) in cacheIndex where !entry.isExpired {
            let filePath = cacheDirectory.appendingPathComponent(entry.fileName)
