...
public struct FARUpdateStatus: Codable {
    public let id: UUID
...
public struct FARUpdateItem: Codable, Identifiable {
    public let id: UUID
...
public class FARUpdateService: ObservableObject {
    ...
    public init() {
        ...
        self.updateStatus = FARUpdateStatus(
    ...
    public func checkForUpdates() async {
        ...
        await MainActor.run {
            self.updateStatus = FARUpdateStatus(
                lastUpdateCheck: Date(),
                lastUpdateCompleted: self.updateStatus.lastUpdateCompleted,
                status: .updating,
                pendingUpdates: 0,
                completedUpdates: self.updateStatus.completedUpdates
            )
        }
        ...
        let hasUpdates = checkMonitorForUpdates()
        ...
        await MainActor.run {
            if hasUpdates {
                self.updateStatus = FARUpdateStatus(
                    ...
                    status: .upToDate,
                    pendingUpdates: 0,
                    completedUpdates: loadCompletedUpdates()
                )
            } else {
                self.updateStatus = FARUpdateStatus(
                    lastUpdateCheck: Date(),
    ...
    private func checkMonitorForUpdates() -> Bool {
    ...
    private func loadCompletedUpdates() -> [FARUpdateItem] {
        ...
        return history.suffix(20).compactMap { update in
            ...
            let changeType: FARUpdateItem.ChangeType = {
            ...
            let impact = analyzeImpact(for: clauseNumber, changeType: changeType)
            let description = generateDescription(for: clauseNumber, changeType: changeType, regulation: regulation)
    ...
    private func analyzeImpact(for clauseNumber: String, changeType: FARUpdateItem.ChangeType) -> FARUpdateItem.ImpactLevel {
    ...
    private func generateDescription(for clauseNumber: String, changeType: FARUpdateItem.ChangeType, regulation: String) -> String {
