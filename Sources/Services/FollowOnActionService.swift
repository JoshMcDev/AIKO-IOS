...
public final class FollowOnActionService {
    ...
    public func executeAction(
        ...
        do {
            ...
            switch action.automationLevel {
            case .manual:
                result = try await executeManualAction(action, acquisitionId: acquisitionId)
            case .semiAutomated:
                result = try await executeSemiAutomatedAction(action, acquisitionId: acquisitionId)
            case .fullyAutomated:
                result = try await executeFullyAutomatedAction(action, acquisitionId: acquisitionId)
            }
    ...
    private func identifyParallelizationOpportunities(levels: [[FollowOnAction]]) -> [ParallelizationOpportunity] {
        levels.enumerated().compactMap { index, level in
            ...
            return ParallelizationOpportunity(
                level: index,
                actions: parallelizable.map { $0.id },
    ...
    private func calculateTimeSaving(actions: [FollowOnAction]) -> TimeInterval {
        ...
        let parallel = actions.map { $0.estimatedDuration }.max() ?? 0
    ...
    private func performAction(
        ...
        case .documentGeneration:
            return try await handleDocumentGeneration(action, preparation: preparation)
        case .requirementGathering:
            return try await handleRequirementGathering(action, preparation: preparation)
        case .vendorManagement:
            return try await handleVendorManagement(action, preparation: preparation)
        case .reviewApproval:
            return try await handleReviewApproval(action, preparation: preparation)
        case .complianceCheck:
            return try await handleComplianceCheck(action, preparation: preparation)
        case .marketResearch:
            return try await handleMarketResearch(action, preparation: preparation)
        case .riskAssessment:
            return try await handleRiskAssessment(action, preparation: preparation)
        case .dataAnalysis:
            return try await handleDataAnalysis(action, preparation: preparation)
        case .communication:
            return try await handleCommunication(action, preparation: preparation)
        case .systemConfiguration:
            return try await handleSystemConfiguration(action, preparation: preparation)
        }
    }
    ...
    private func mapToAgentActionType(_ category: ActionCategory) -> AgentAction.ActionType {
        ...
        case .vendorManagement:
            return .identifyVendors
        case .reviewApproval:
            return .submitForApproval
        case .complianceCheck:
            return .monitorCompliance
        case .marketResearch:
            return .gatherMarketResearch
        default:
            return .gatherMarketResearch
        }
    }
    ...
    private func notifyUserOfManualTask(_ task: UserTask) async {
        // Send notification to user
    }
    
    private func notifyActionAvailable(_ action: FollowOnAction) async {
...
public enum FollowOnActionError: LocalizedError {
    ...
    public var errorDescription: String? {
        ...
        case .dependenciesNotSatisfied(let deps):
            return "Dependencies not satisfied: \(deps.count) actions must complete first"
        case .executionFailed(let reason):
            return "Action execution failed: \(reason)"
        case .invalidContext:
            return "Invalid action context"
        }
    }
}
...
public struct ContextUpdate {
    ...
    func apply(to context: String) -> FollowOnActionContext {
...
public struct ActionPreparation {
    ...
    public let acquisitionData: Acquisition
...
extension DependencyValues {
    public var followOnActionService: FollowOnActionService {
