...
actor DocumentCacheStorage {
    ...
    func getStatistics() -> CacheStatistics {
        ...
        let estimatedSize = Int64(totalDocuments * 50_000 + totalAnalyses * 10_000)
        ...
        let typeAccessCounts = documentCache.reduce(into: [DocumentType: Int]()) { result, entry in
            if case .standard(let type) = entry.key.documentCategory {
    ...
    func optimizeForMemory() {
        ...
        accessOrder = Array(Set(
            documentCache.keys.map { $0.hashValue.description } +
            analysisCache.keys
        ))
    }
}
...
extension DependencyValues {
    public var documentCacheService: DocumentCacheService {
