...
public struct DocumentGenerationPreloader {
    ...
    public func preloadData(
        ...
        return try await withThrowingTaskGroup(of: PreloadComponent.self) { group in
            // Load user profile
            group.addTask {
                let profile = try? await self.userProfileService.loadProfile()
            ...
            for documentType in documentTypes {
                group.addTask {
                    let template = await self.loadTemplate(for: documentType)
            ...
            for dfDocumentType in dfDocumentTypes {
                group.addTask {
                    let template = try? await self.dfTemplateService.loadTemplate(dfDocumentType)
            ...
            for documentType in documentTypes {
                group.addTask {
                    let prompt = await self.loadSystemPrompt(for: documentType)
            ...
            for dfDocumentType in dfDocumentTypes {
                group.addTask {
                    let prompt = await self.loadDFSystemPrompt(for: dfDocumentType)
...
public struct DocumentGenerationPreloaderKey: DependencyKey {
