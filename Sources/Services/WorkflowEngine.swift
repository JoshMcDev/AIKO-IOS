    ...
    public static var liveValue: WorkflowEngine {
        ...
        return WorkflowEngine(
            ...
            generatePrompts: { context in
                ...
                case .suggestingDocuments:
                    ...
                    for doc in suggestedDocs {
                        ...
                        let validation = documentDependencyService.validateDependencies(
                            try await fetchPreviousDocuments(for: context.acquisitionId),
                            doc
                        )
                        ...
                        if !validation.isValid {
                            ...
                            if !validation.missingDocuments.isEmpty {
                                promptText += " (Missing: \(validation.missingDocuments.map { $0.shortName }.joined(separator: ", ")))"
                            }
            ...
            getNextSteps: { context in
                return context.currentState.nextStates
            },
            ...
            extractDataFromDocument: { document in
                ...
                return documentDependencyService.extractDataForDependents(document)
            },
...
private func saveWorkflowContext(_ context: WorkflowContext, to acquisitionId: UUID) async throws {
    // Save workflow context to Core Data or persistent storage
    // This would update the acquisition entity with workflow data
}

private func loadWorkflowContext(for acquisitionId: UUID) async throws -> WorkflowContext {
    // Load workflow context from Core Data
    throw WorkflowError.notImplemented
}

private func loadWorkflowData(from acquisition: Acquisition) async throws -> (currentState: WorkflowState, steps: [WorkflowStep], automationSettings: AutomationSettings, collectedData: CollectedData, pendingApprovals: [WorkflowStep]) {
    // Extract workflow data from Core Data
    throw WorkflowError.notImplemented
}

private func generatePromptsForState(_ state: WorkflowState, context: WorkflowContext) async throws -> [SuggestedPrompt] {
    // Generate state-specific prompts
    return []
}

private func updateAcquisitionData(_ acquisitionId: UUID, with data: CollectedData) async throws {
...
private func identifyMissingDataFields(_ context: WorkflowContext) async throws -> [(key: String, displayName: String, isRequired: Bool)] {
    // Identify missing required fields
    return []
}

private func processAndExtractData(from response: String, existingData: CollectedData) async throws -> CollectedData {
    // Process LLM response and extract structured data
    return existingData
}

private func parseDocumentContent(_ document: GeneratedDocument) async throws -> CollectedData {
    // Parse document and extract data
    return CollectedData()
}

private func fetchPreviousDocuments(for acquisitionId: UUID) async throws -> [GeneratedDocument] {
    // Fetch previously generated documents
    return []
}

private func extractAcquisitionData(from acquisition: Acquisition?) async throws -> CollectedData {
    // Extract data from acquisition entity
    return CollectedData()
}

private func extractProfileData(from profile: UserProfile?) -> CollectedData {
    // Extract data from user profile
    return CollectedData()
}

private func extractDataFromDocuments(_ documents: [GeneratedDocument]) async throws -> CollectedData {
    // Extract and merge data from multiple documents
    return CollectedData()
}

private func buildTemplateVariables(acquisitionData: CollectedData, profileData: CollectedData, previousDocsData: CollectedData, documentType: DocumentType) -> [String: String] {
...
enum WorkflowError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .missingRequiredData:
            return "Missing required data for operation"
        case .notImplemented:
...
extension DependencyValues {
    public var workflowEngine: WorkflowEngine {
