...
public class AdaptiveDataExtractor: @unchecked Sendable {
    ...
    public func extractAdaptively(
        ...
        let discoveredObjects = try await discoverNewFields(
            in: document,
            excluding: extractedObjects.map { $0.fieldName }
        )
        ...
        return AdaptiveExtractionResult(
            ...
            documentSignature: documentSignature,
            appliedPatterns: applicablePatterns.map { $0.patternName },
            ...
            for mapping in pattern.fieldMappings {
                if let value = try extractFieldUsingMapping(mapping, from: document) {
                    let context = DocumentContext(
                        ...
                        lineNumber: nil,
                        surroundingText: nil,
                        relatedFields: pattern.fieldMappings.map { $0.standardFieldName }
                    )
    ...
    private func updatePatternLearning(
        ...
        for group in objectGroups {
            ...
            var matchedPattern: LearnedPattern? = nil
    ...
    private func findApplicablePatterns(for signature: String) -> [LearnedPattern] {
        return learnedPatterns.values.filter { pattern in
    ...
    private func patternMatches(_ objects: [DynamicValueObject], pattern: LearnedPattern) -> Bool {
        let objectFieldNames = Set(objects.map { $0.fieldName })
        let patternFieldNames = Set(pattern.fieldMappings.map { $0.standardFieldName })
...
class PatternLearner {
    func analyzePatterns(in documents: [ParsedDocument]) -> [LearnedPattern] {
