        ...
        Reduce { state, action -> Effect<Action> in
            ...
            case let .acquisitionSaved(id):
                ...
                if let session = state.conversationSession {
                    return .run { [promptingEngine = self.promptingEngine, session = session, parsedDocuments = state.parsedDocuments, collectedData = state.collectedData] _ in
...
public extension DependencyValues {
    ...
    var documentParserEnhanced: DocumentParserEnhanced {
...
extension RequirementsData {
    func value(for field: RequirementField) -> Any? {
        ...
        case .vendorName: vendorInfo?.name
        case .vendorUEI: vendorInfo?.uei
        case .vendorCAGE: vendorInfo?.cage
        case .performanceLocation: placeOfPerformance
        case .contractType: acquisitionType
        case .setAsideType: setAsideType
        case .technicalSpecs: technicalRequirements
        case .requiredDate: requiredDate
        case .specialConditions: specialConditions
        case .justification: businessJustification
        case .fundingSource, .requisitionNumber, .costCenter, .accountingCode,
            ...
            nil // These fields are not yet in the data model
