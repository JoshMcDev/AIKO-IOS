...
public final class SettingsViewModel {
    ...
    public func updateAppSetting<T>(_ keyPath: WritableKeyPath<AppSettingsData, T>, value: T) async {
        ...
        if keyPath == \.autoSaveInterval, let interval = value as? Int {
            ...
            // Safe generic value assignment using type constraints
            if let safeValue = clampedInterval as? T {
                settingsData.appSettings[keyPath: keyPath] = safeValue
            } else {
                // This should never happen if T is Int, but provides safe fallback
                validationError = "Type mismatch: Expected Int for autoSaveInterval"
                isLoading = false
                saveStatus = .error
                return
            }
        } else if keyPath == \.autoSaveInterval {
            // Handle case where value is not Int for interval setting
            validationError = "Invalid type for autoSaveInterval: Expected Int"
            isLoading = false
            saveStatus = .error
            return
