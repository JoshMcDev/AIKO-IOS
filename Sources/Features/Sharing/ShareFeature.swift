...
public struct ShareFeature {
    ...
    public var body: some ReducerOf<Self> {
        Reduce { state, action in
            ...
            case .selectDocument(let id):
                state.selectedDocumentIds.insert(id)
                return .none
                
            case .deselectDocument(let id):
                state.selectedDocumentIds.remove(id)
                return .none
                
            case .selectAllDocuments(let ids):
            ...
            case .shareItemsPrepared(let items):
            ...
            case .setShowingShareSheet(let showing):
    ...
    private func prepareShareItems(
        ...
        switch mode {
        case .singleDocument:
            return ["Document content for single share"]
            
        case .multipleDocuments:
            return documentIds.map { "Document \($0.uuidString)" }
            
        case .acquisition(let id):
            return ["Acquisition \(id.uuidString) with \(documentIds.count) documents"]
            
        case .contractFile(let id):
            return ["Contract file for acquisition \(id.uuidString)"]
...
public enum ShareMode: Equatable {
    ...
    public var title: String {
        ...
        case .acquisition:
            return "Share Acquisition Documents"
        case .contractFile:
            return "Share Contract File"
        }
    }
...
extension ShareFeature.State {
    ...
    public var shareDescription: String {
        ...
        case .singleDocument:
            return "Sharing 1 document"
        case .multipleDocuments:
            return "Sharing \(selectionCount) documents"
        case .acquisition:
            return "Sharing acquisition documents (\(selectionCount) selected)"
        case .contractFile:
            return "Sharing contract file"
        }
    }
