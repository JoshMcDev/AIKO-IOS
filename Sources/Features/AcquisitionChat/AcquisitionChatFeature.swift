...
public struct AcquisitionChatFeature {
    ...
    public struct State: Equatable {
        ...
        public var gatheredRequirements: RequirementsData = RequirementsData()
        ...
        public var inputPlaceholder: String {
            switch currentPhase {
            case .initial:
                return "Describe the product or service you need..."
            case .gatheringBasics:
                if gatheredRequirements.estimatedValue.isEmpty {
                    return "Enter the estimated dollar value (e.g., $50,000)..."
                } else if gatheredRequirements.performancePeriod.isEmpty {
                    return "Enter the performance period (e.g., 12 months)..."
                } else if gatheredRequirements.businessNeed.isEmpty {
                    return "Describe why this is needed..."
                } else {
                    return "Provide additional details..."
                }
            case .gatheringDetails:
                if gatheredRequirements.technicalRequirements.isEmpty {
                    return "Describe technical requirements or type 'skip'..."
                } else {
                    return "Add more details or type 'skip' to continue..."
                }
            case .analyzingRequirements:
                return "Please wait while I analyze your requirements..."
            case .confirmingPredictions:
                return "Confirm if the values are correct or suggest changes..."
            case .readyToGenerate:
                return "Type 'generate all' or select specific documents..."
            }
        }
    }
    ...
    public struct RequirementsData: Equatable {
        ...
        public var hasMinimumInfo: Bool {
            return !productOrService.isEmpty && 
            ...
            case .enhancePrompt:
                ...
                return .run { [input = state.currentInput] send in
                    do {
                        ...
                        if case .text(let enhancedText, _) = result.content.first {
