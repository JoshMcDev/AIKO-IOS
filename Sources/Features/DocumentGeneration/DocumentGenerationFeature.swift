...
public struct DocumentGenerationFeature {
    ...
    public var body: some ReducerOf<Self> {
        ...
        Reduce { state, action in
            ...
            case .analysis(.requirementsAnalyzed(_, let recommendedDocs)):
                // Update status when analysis completes
                return .send(.status(.setRecommendedDocuments(recommendedDocs)))
                
            case .analysis(.documentUploaded(let response, let recommendedDocs)):
                ...
                return .run { [requirements = enhancedRequirements,
                    ...
                    do {
                        ...
                        let context = ActionContext(
                            userId: (try? await userProfileService.loadProfile())?.id.uuidString ?? "anonymous",
