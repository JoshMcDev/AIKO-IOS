...
struct LLMProviderSettingsFeature {
    ...
    var body: some Reducer<State, Action> {
        Reduce { state, action in
            ...
            case let .alert(.presented(.confirmSetActive(providerId))):
                state.alert = nil
                return .send(.setActiveProvider(providerId))

            case let .alert(.presented(.confirmRemove(providerId))):
                state.alert = nil
                return .run { send in
                    do {
    ...
    struct State: Equatable {
        ...
        var canSave: Bool {
            if provider.id == "local" {
                ...
                !apiKey.isEmpty && !customEndpoint.isEmpty && !deploymentName.isEmpty
            } else {
                !apiKey.isEmpty
            }
        }
