...
@Reducer
public struct UnifiedChatFeature {
    @ObservableState
    public struct State: Equatable {
        ...
        public init(
            ...
            switch initialMode {
            case .guided:
                self.acquisitionState = AcquisitionChatFeature.State()
            case .agentic:
                self.agenticState = AgenticChatFeature.State()
            case .hybrid:
                self.acquisitionState = AcquisitionChatFeature.State()
                self.agenticState = AgenticChatFeature.State()
            }
    ...
    private func handleUnifiedSendMessage(
        ...
        switch state.currentMode {
        case .guided:
            return .send(.acquisition(.sendMessage))
            
        case .agentic:
            return .send(.agentic(.sendMessage))
    ...
    private func syncContextToChildren(state: State) -> Effect<Action> {
...
public enum ChatMode: String, CaseIterable, Equatable {
    ...
    var icon: String {
        switch self {
        case .guided: return "questionmark.circle"
        case .agentic: return "bolt.circle"
        case .hybrid: return "arrow.triangle.2.circlepath.circle"
        }
    }
    ...
    var description: String {
        switch self {
        case .guided:
            return "Step-by-step requirement gathering"
        case .agentic:
            return "Autonomous task execution"
        case .hybrid:
            return "Combined guided and autonomous operation"
        }
    }
}
...
struct UnifiedInputBar: View {
    ...
    var placeholderText: String {
        ...
        case .guided:
            return "Describe your requirements..."
        case .agentic:
            return "What task should I execute?"
        case .hybrid:
            return "Ask questions or give commands..."
        }
    }
}
