...
public actor SessionEngine: Sendable {
    ...
    public func updatePageStatus(id: SessionPage.ID, status: PageProcessingStatus) async throws -> ScanSession {
        ...
        if case .inProgress(_, let total) = currentSession.batchOperationState {
    ...
    public func pauseBatchProcessing() async throws -> ScanSession {
        if case .inProgress(let completed, let total) = currentSession.batchOperationState {
    ...
    public func resumeBatchProcessing() async throws -> ScanSession {
        if case .paused(let completed, let total) = currentSession.batchOperationState {
            currentSession.batchOperationState = .inProgress(completedCount: completed, total: total)
            currentSession.status = .processing
            currentSession.touch()
...
extension SessionEngine: DependencyKey {
    public static let liveValue: SessionEngine = SessionEngine()
    public static let testValue: SessionEngine = SessionEngine()
}
...
public actor SessionStorage: Sendable {
